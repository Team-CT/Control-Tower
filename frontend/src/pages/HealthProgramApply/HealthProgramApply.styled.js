...
color: ${({ theme }) => theme.text.inverse || 'white'};
background: ${(props) => {
    if ((props.$type || '').includes('상담')) return props.theme.status.success || '#10b981';
    if ((props.$type || '').includes('운동')) return props.theme.status.warning || '#f59e0b';
    if ((props.$type || '').includes('휴식')) return props.theme.colors.secondary || '#8b5cf6';
...
font - size: 12px;
font - weight: 600;
color: ${(props) => (props.$status === 'approved' ? (props.theme.status.success || '#047857') : (props.theme.status.error || '#dc2626'))};
background: ${(props) => (props.$status === 'approved' ? `${props.theme.status.success}15` : `${props.theme.status.error}15`)};
border - radius: 6px;
white - space: nowrap;
`;
