...
public class ProtestApplyServiceImpl implements ProtestApplyService {
        ...
        public ProtestDto.ListResponse applyProtest(
                        ...
                        targetAttendance = attendanceRepository
                                        ...
                                        .orElseGet(() -> {
                                                ...
                                                Attendance newAttendance = Attendance.builder()
                                                                ...
                                                                .attendanceStatus(CommonEnums.AttendanceStatus.ABSENT) // 기본값:
                                                                                                                       // 결근
                                                                .build();
                ...
                if (request.getProtestRequestInTime() != null && !request.getProtestRequestInTime().isEmpty()) {
                        requestInTime = LocalTime.parse(request.getProtestRequestInTime(),
                                        DateTimeFormatter.ofPattern("HH:mm"));
                }

                if (request.getProtestRequestOutTime() != null && !request.getProtestRequestOutTime().isEmpty()) {
                        requestOutTime = LocalTime.parse(request.getProtestRequestOutTime(),
                                        DateTimeFormatter.ofPattern("HH:mm"));
                }
                ...
                if (request.getAttendanceType() != null && !request.getAttendanceType().isEmpty()) {
                        try {
                                attendanceType = CommonEnums.AttendanceType
                                                .valueOf(request.getAttendanceType().toUpperCase());
        ...
        private ProtestDto.ListResponse convertToListResponse(ProtestApply entity) {
                return ProtestDto.ListResponse.builder()
                                ...
                                .attendanceType(entity.getAttendanceType() != null ? entity.getAttendanceType().name()
                                                : null)
                                .applicantName(entity.getProtestApplyApplicant().getEmpName())
                                .approverName(entity.getProtestApplyApprover() != null
                                                ? entity.getProtestApplyApprover().getEmpName()
                                                : null)
        ...
        public void approveProtestByAdmin(Long protestId, String approverId) {
                ...
                // 4. 신청한 근무 유형(attendanceType)을 AttendanceStatus로 변환
                CommonEnums.AttendanceStatus newStatus = convertTypeToStatus(protestApply.getAttendanceType());
                log.info("attendanceType: {} -> attendanceStatus: {}", protestApply.getAttendanceType(), newStatus);

                // 5. 실제 근태 기록 업데이트 - 신청한 유형에 맞는 상태로 변경
                ...
                // 6. Dirty Checking으로 자동 저장
                log.info("정정 신청 승인 완료 - protestId: {}, 변경된 상태: {}", protestId, newStatus);
        }
        ...
        public void rejectProtestByAdmin(Long protestId, String approverId, String cancelReason) {
                ...
                // 5. 반려 시 휴가 유형이면 연차 복원
                CommonEnums.AttendanceType attendanceType = protestApply.getAttendanceType();
                if (attendanceType == CommonEnums.AttendanceType.LEAVE
                                || attendanceType == CommonEnums.AttendanceType.HALF_LEAVE) {
                        Emp applicant = protestApply.getProtestApplyApplicant();
                        // 반차는 0.5일, 휴가는 1.0일 복원
                        double daysToRestore = (attendanceType == CommonEnums.AttendanceType.HALF_LEAVE) ? 0.5 : 1.0;
                        applicant.restoreLeaveCount(daysToRestore);
                        log.info("반려로 인한 연차 복원 - empId: {}, 복원 일수: {}", applicant.getEmpId(), daysToRestore);
                }
        ...
        /**
         * AttendanceType -> AttendanceStatus 변환
         * 근태정정 신청 시 사용자가 선택한 근무 유형을 실제 근태 상태로 매핑
         */
        private CommonEnums.AttendanceStatus convertTypeToStatus(CommonEnums.AttendanceType type) {
                if (type == null)
                        return CommonEnums.AttendanceStatus.PRESENT;
                switch (type) {
                        case NORMAL:
                                return CommonEnums.AttendanceStatus.PRESENT; // 정상출근
                        case LATE:
                                return CommonEnums.AttendanceStatus.LATE; // 지각
                        case EARLY_LEAVE:
                                return CommonEnums.AttendanceStatus.EARLY_LEAVE; // 조퇴
                        case ABSENT:
                                return CommonEnums.AttendanceStatus.ABSENT; // 결근
                        case LEAVE:
                                return CommonEnums.AttendanceStatus.LEAVE; // 휴가
                        case HALF_LEAVE:
                                return CommonEnums.AttendanceStatus.HALF_DAY; // 반차
                        default:
                                return CommonEnums.AttendanceStatus.PRESENT;
                }
        }
        ...
        private ProtestDto.AdminListResponse convertToAdminListResponse(ProtestApply entity) {
                ...
                return ProtestDto.AdminListResponse.builder()
                                ...
                                .departmentName(applicant.getDepartmentId() != null
